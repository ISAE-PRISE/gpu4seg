...
/*--------------------------- segnet_arch6_implementations.h ------------------------
|  File segnet_arch6_implementations.h
|
|  Description: Implementations of our 6th Segnet architecture 
|
|  Version: 1.0
*-----------------------------------------------------------------------------------*/

#ifndef SEGNET_ARCH6_IMPLEMENTATIONS_H_
#define SEGNET_ARCH6_IMPLEMENTATIONS_H_


//#define EVENT_RECORD_CYCLES 
#define L2_CACHE_LOCK 


/* SegnetImplementationOriginalTraditionalArch6
 *
 * Description: Executes a modified Segnet CNN considering 3 enconder-decoder layers 
 *		 and output channels of 32, 64 and 128 and 2, 3 and 3 convolutions respectively.
 ...
 *
 * */
void SegnetImplementationOriginalTraditionalArch6(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
  ...
  // Number of encoder-decoder layers (must be 2 or higher)
  ...
  unsigned threads_per_block_conv[NB_LAYERS] = {32, 64, 128};
  ...
  for (int j = 0; j < nIter; j++){
	...
	for (int cnt_layer = 1; cnt_layer < NB_LAYERS; cnt_layer++){
		...
		if(layer_num < 2)
			...
			convA_norm_ReLu_CUDA<128, 3, 128> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_max_pooled[layer_num-1], d_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels,  fps_encoder[conv_num].out_channels, d_mean_encoder[conv_num], d_var_encoder[conv_num], d_weights_encoder[conv_num], d_bias_norm_encoder[conv_num], EPSILON);				
	...
	for (int cnt_layer = NB_LAYERS-1; cnt_layer >= 0; cnt_layer--, layer_num--){
		...
		if(cnt_layer > 1){
			...
			convA_norm_ReLu_CUDA<32, 3, 32> <<<blocks_per_grid_conv[conv_num-1], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_mean_decoder[conv_num], d_var_decoder[conv_num], d_weights_decoder[conv_num], d_bias_norm_decoder[conv_num], EPSILON);
		}else{
...
/* SegnetImplementationIntegratedTraditionalArch6
 *
 * Description: Executes a modified Segnet CNN considering 3 enconder-decoder layers 
 *		 and output channels of 32, 64 and 128 and 2, 3 and 3 convolutions respectively.
 *		 Convolutions are performed using the traditional convolution operation. 	
 ...
 *
 * */
void SegnetImplementationIntegratedTraditionalArch6(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
  ...
  for (int j = 0; j < nIter; j++){
	...
	for (int cnt_layer = 1; cnt_layer < NB_LAYERS; cnt_layer++){
		...
		if(layer_num < 2)
			...
			convA_ReLu_CUDA<128, 3, 128> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_max_pooled[layer_num-1], d_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels,  fps_encoder[conv_num].out_channels, d_bias_encoder[conv_num]);				
	...
	for (int cnt_layer = NB_LAYERS-1; cnt_layer >= 0; cnt_layer--, layer_num--){
		...
		// Perform covolution after upsampling
		if(cnt_layer > 1){
			convA_ReLu_CUDA<128, 3, 128> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_max_unpooled[layer_num], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_bias_decoder[conv_num]);
		}else if(cnt_layer > 0){
...
/* SegnetImplementationOriginalWinograd2x2Arch6
 *
 * Description: Executes the Segnet CNN  enconder-decoder layers and output 
 *		 channels of 32, 64 and 128 and 2, 3 and 3 convolutions respectively.
 ...
 *
 * */
void SegnetImplementationOriginalWinograd2x2Arch6(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
  ...
  for (int j = 0; j < nIter; j++){
	...
	for (int cnt_layer = 1; cnt_layer < NB_LAYERS; cnt_layer++){
		...
		if(layer_num < 2)
			wino2_conv2x2_norm_ReLu_CUDA<32, 32> <<<2*blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_max_pooled[layer_num-1], d_wino_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels, fps_encoder[conv_num].out_channels, d_mean_encoder[conv_num], d_var_encoder[conv_num], d_weights_encoder[conv_num], d_bias_norm_encoder[conv_num], EPSILON);
		else if(layer_num < 3)
	...
	for (int cnt_layer = NB_LAYERS-1; cnt_layer >= 0; cnt_layer--, layer_num--){
		...
		/* **************  Deconvolution Loop ************** */
		 if(cnt_layer > 1){
			...
			wino3_conv2x2_norm_ReLu_CUDA<32, 64> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_wino_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_mean_decoder[conv_num], d_var_decoder[conv_num], d_weights_decoder[conv_num], d_bias_norm_decoder[conv_num], EPSILON);
		}else{
 ...
 /* SegnetImplementationIntegratedWinograd2x2Arch6
 *
 * Description: Executes the Segnet CNN  enconder-decoder layers and output 
 *		 channels of 32, 64 and 128 and 2, 3 and 3 convolutions respectively.
 ...
 *
 * */
void SegnetImplementationIntegratedWinograd2x2Arch6(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
  ...
  for (int j = 0; j < nIter; j++){
	...
	// Perform image*filter convolution for the resized image
  	wino_conv2x2_ReLu_CUDA<32, 3> <<<size_Img_X[layer_num]*size_Img_Y[layer_num]/WINO_TILE_SIZE, 32, 0, m0_stream>>>(d_features_encoder_l[conv_num], d_input_img_resized, d_wino_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels, fps_encoder[conv_num].out_channels, d_bias_encoder[conv_num]);
  		
  	/* **************  Layer 1 - Convolution 2 ************** */	
	conv_num = 1;

	// Perform image*filter convolution 
	wino_conv2x2_ReLu_CUDA<32, 32> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_features_encoder_l[conv_num-1], d_wino_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels, fps_encoder[conv_num].out_channels, d_bias_encoder[conv_num]);
	...
	for (int cnt_layer = 1; cnt_layer < NB_LAYERS; cnt_layer++){
		...
		// Perform image*filter convolution for the current feature
		if(layer_num < 2)
			wino2_conv2x2_ReLu_CUDA<32, 32> <<<2*blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_max_pooled[layer_num-1], d_wino_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels, fps_encoder[conv_num].out_channels, d_bias_encoder[conv_num]);
		else if(layer_num < 3)
	...
	for (int cnt_layer = NB_LAYERS-1; cnt_layer >= 0; cnt_layer--, layer_num--){
		...
		if(cnt_layer > 1){
			...
			wino3_conv2x2_ReLu_CUDA<32, 64> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_wino_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_bias_decoder[conv_num]);
		}else{
