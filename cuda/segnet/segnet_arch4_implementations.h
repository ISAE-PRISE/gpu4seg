...
/*--------------------------- segnet_arch4_implementations.h ------------------------
|  File segnet_arch4_implementations.h
|
|  Description: Implementations of our 4th Segnet architecture 
|
|  Version: 1.0
*-----------------------------------------------------------------------------------*/

#ifndef SEGNET_ARCH4_IMPLEMENTATIONS_H_
#define SEGNET_ARCH4_IMPLEMENTATIONS_H_


//#define EVENT_RECORD_TIME 


/* SegnetImplementationOriginalTraditionalArch4
 *
 ...
 *
 * */
void SegnetImplementationOriginalTraditionalArch4(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
  ...
  unsigned threads_per_block_conv[NB_LAYERS] = {64, 128, 256};
  ...
  for (int j = 0; j < nIter; j++){
	...
	for (int cnt_layer = 1; cnt_layer < NB_LAYERS; cnt_layer++){
		...
		if(layer_num < 2)
			...
			convA_norm_ReLu_CUDA<256, 3, 256> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_max_pooled[layer_num-1], d_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels,  fps_encoder[conv_num].out_channels, d_mean_encoder[conv_num], d_var_encoder[conv_num], d_weights_encoder[conv_num], d_bias_norm_encoder[conv_num], EPSILON);				
	...
	for (int cnt_layer = NB_LAYERS-1; cnt_layer >= 0; cnt_layer--, layer_num--){
		...
		if(cnt_layer > 1){
			...
			convA_norm_ReLu_CUDA<128, 3, 128> <<<blocks_per_grid_conv[conv_num-1], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_mean_decoder[conv_num], d_var_decoder[conv_num], d_weights_decoder[conv_num], d_bias_norm_decoder[conv_num], EPSILON);
...
/* SegnetImplementationIntegratedTraditionalArch4
 *
 * Description: Executes a modified Segnet CNN considering 3 enconder-decoder layers 
 *		 and output channels of 64, 128 and 256 and 2, 3 and 3 convolutions respectively.
 *		 Convolutions are performed using the traditional convolution operation. 	
 ...
 *
 * */
void SegnetImplementationIntegratedTraditionalArch4(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
  ...
  unsigned int size_input_Img = width_img * height_img * channels_img; // Image	
  unsigned int mem_size_input_Img = sizeof(uint8_t) * size_input_Img;	
  unsigned int size_Img_Ch = width_img * height_img; // Image channel	
  unsigned int mem_size_Img_Ch = sizeof(uint8_t) * size_Img_Ch;
  ...
  uint8_t *d_input_img, *d_input_img_resized; 
  checkCudaErrors(cudaMalloc(reinterpret_cast<void **>(&d_input_img), mem_size_input_Img));
  checkCudaErrors(cudaMalloc(reinterpret_cast<void **>(&d_input_img_resized), mem_size_Img_Ch*NB_CHANNELS));
  ...
  for (int j = 0; j < nIter; j++){
	...
	for (int cnt_layer = 1; cnt_layer < NB_LAYERS; cnt_layer++){
		...
		if(layer_num < 2)
			...
			convA_ReLu_CUDA<256, 3, 256> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_max_pooled[layer_num-1], d_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels,  fps_encoder[conv_num].out_channels, d_bias_encoder[conv_num]);				
	...
	for (int cnt_layer = NB_LAYERS-1; cnt_layer >= 0; cnt_layer--, layer_num--){
		...
		// Perform covolution after upsampling
		if(cnt_layer > 1){
			convA_ReLu_CUDA<256, 3, 256> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_max_unpooled[layer_num], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_bias_decoder[conv_num]);
		}else if(cnt_layer > 0){
		...
		if(cnt_layer > 1){
			...
			convA_ReLu_CUDA<128, 3, 128> <<<blocks_per_grid_conv[conv_num-1], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_bias_decoder[conv_num]);
...
/* SegnetImplementationOriginalWinograd2x2Arch4
 *
 * Description: Executes the Segnet CNN  enconder-decoder layers and output 
 *		 channels of 64, 128 and 256 and 2, 3 and 3 convolutions respectively.
 ...
 *
 * */
void SegnetImplementationOriginalWinograd2x2Arch4(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
 ...
 /* SegnetImplementationIntegratedWinograd2x2Arch4
 *
 * Description: Executes the Segnet CNN  enconder-decoder layers and output 
 *		 channels of 64, 128 and 256 and 2, 3 and 3 convolutions respectively.
 ...
 *
 * */
void SegnetImplementationIntegratedWinograd2x2Arch4(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
