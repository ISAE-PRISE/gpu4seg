...
/*--------------------------- segnet_arch2_implementations.h ------------------------
|  File segnet_arch2_implementations.h
|
|  Description: Implementations of our 2nd Segnet architecture 
|
|  Version: 1.0
*-----------------------------------------------------------------------------------*/

#ifndef SEGNET_ARCH2_IMPLEMENTATIONS_H_
#define SEGNET_ARCH2_IMPLEMENTATIONS_H_
...
/* SegnetImplementationOriginalTraditionalArch2
 *
 * Description: Executes a modified Segnet CNN considering 5 enconder-decoder layers
 *		 with output channels of 32, 64, 128, 256, 512 and 2, 2, 3, 3, 3 
 ...
 *
 * */
 void SegnetImplementationOriginalTraditionalArch2(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
  ...
  unsigned threads_per_block_conv[NB_LAYERS] = {32, 64, 128, 256, 512};
  ...
  for (int j = 0; j < nIter; j++){
	...
	for (int cnt_layer = 1; cnt_layer < NB_LAYERS; cnt_layer++){
		...
		if(layer_num < 2)
			...
			convA_norm_ReLu_CUDA<512, 3, 512> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_max_pooled[layer_num-1], d_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels,  fps_encoder[conv_num].out_channels, d_mean_encoder[conv_num], d_var_encoder[conv_num], d_weights_encoder[conv_num], d_bias_norm_encoder[conv_num], EPSILON);
	...
	for (int cnt_layer = NB_LAYERS-1; cnt_layer >= 0; cnt_layer--, layer_num--){
		...
		if(cnt_layer > 3){
			...
			convA_norm_ReLu_CUDA<256, 3, 256> <<<blocks_per_grid_conv[conv_num-1], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_mean_decoder[conv_num], d_var_decoder[conv_num], d_weights_decoder[conv_num], d_bias_norm_decoder[conv_num], EPSILON);
...
/* SegnetImplementationIntegratedTraditionalArch2
 *
 * Description: Executes a modified Segnet CNN considering 5 enconder-decoder layers
 *		 with output channels of 32, 64, 128, 256, 512 and 2, 2, 3, 3, 3 
 *		 convolutions respectively.
 ...
 *
 * */
void SegnetImplementationIntegratedTraditionalArch2(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
  ...
  // The rest of convolutions
  unsigned threads_per_block_conv[NB_CONVs] = {32, 64, 128, 256, 512};
  unsigned blocks_per_grid_conv[NB_CONVs];
  ...
  for (int j = 0; j < nIter; j++){
	...
	for (int cnt_layer = NB_LAYERS-1; cnt_layer >= 0; cnt_layer--, layer_num--){
		...
		if(cnt_layer > 3){
			...
			convA_ReLu_CUDA<32, 3, 32> <<<blocks_per_grid_conv[layer_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_max_unpooled[layer_num], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_bias_decoder[conv_num]);
		...
		if(cnt_layer > 3){
			...
			convA_ReLu_CUDA<32, 3, 32> <<<blocks_per_grid_conv[layer_num-1], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_bias_decoder[conv_num]);
		}else{
...
/* SegnetImplementationOriginalWinograd2x2Arch2
 *
 * Description: Executes a modified Segnet CNN considering 5 enconder-decoder layers
 *		 with output channels of 32, 64, 128, 256, 512 and 2, 2, 3, 3, 3 
 *		 convolutions respectively. Convolutions are made via Winograd. 
 *		 This implementation performs the batch normalization. Float32 is used.
 *
 * Parameter:   
 *		- const dim3 &dimsImg: Dimensions of the resized input image 
 *		- const dim3 &dimsFilter: Dimensions of the filters
 ...
 *
 * */
void SegnetImplementationOriginalWinograd2x2Arch2(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
...
/* SegnetImplementationIntegratedWinograd2x2Arch2
 *
 * Description: Executes a modified Segnet CNN considering 5 enconder-decoder layers
 *		 with output channels of 32, 64, 128, 256, 512 and 2, 2, 3, 3, 3 
 *		 convolutions respectively. Convolutions are made via Winograd.  
 *		 The filter weights integrate the batch normalization. Float32 is used.	
 *
 * Parameter:   
 *		- const dim3 &dimsImg: Dimensions of the resized input image 
 *		- const dim3 &dimsFilter: Dimensions of the filters
 ...
 *
 * */
void SegnetImplementationIntegratedWinograd2x2Arch2(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
