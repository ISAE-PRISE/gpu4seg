...
/*--------------------------- segnet_arch3_implementations.h ------------------------
|  File segnet_arch3_implementations.h
|
|  Description: Implementations of our 3rd Segnet architecture 
|
|  Version: 1.0
*-----------------------------------------------------------------------------------*/

#ifndef SEGNET_ARCH3_IMPLEMENTATIONS_H_
#define SEGNET_ARCH3_IMPLEMENTATIONS_H_


//#define EVENT_RECORD_TIME 


/* SegnetImplementationOriginalTraditionalArch3
 *
 * Description: Executes a modified Segnet CNN considering 4 enconder-decoder layers
 *		 with output channels of 64, 128, 256, 256 and 2, 2, 3, 3
 ...
 *
 * */
 void SegnetImplementationOriginalTraditionalArch3(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
  ...
  const unsigned CONV_2_LAYER[NB_CONVs] = {0, 0, 1, 1, 2, 2, 2, 3, 3, 3};
  const unsigned CONV_PER_LAYER[NB_LAYERS] = {NB_CONV_LAYER1, NB_CONV_LAYER2, NB_CONV_LAYER3, NB_CONV_LAYER4};
  ...
  unsigned pooled_index[NB_LAYERS] = {1, 3, 6, 9};
  ...
  unsigned threads_per_block_conv[NB_LAYERS] = {64, 128, 256, 256};
  ...
  for (int j = 0; j < nIter; j++){
	...
	/* **************  Decoder  ************** */
	/* **************  Layer 4/3/2/1 - Convolution 10-1 ************** */
...
/* SegnetImplementationIntegratedTraditionalArch3
 *
 * Description: Executes a modified Segnet CNN considering 4 enconder-decoder layers
 *		 with output channels of 64, 128, 256, 256 and 2, 2, 3, 3
 *		 convolutions respectively.
 ...
 *
 * */
void SegnetImplementationIntegratedTraditionalArch3(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
  ...
  // The rest of convolutions
  unsigned threads_per_block_conv[NB_CONVs] = {64, 128, 256, 256};
  unsigned blocks_per_grid_conv[NB_CONVs];
...
/* SegnetImplementationOriginalWinograd2x2Arch3
 *
 * Description: Executes a modified Segnet CNN considering 4 enconder-decoder layers
 *		 with output channels of 64, 128, 256, 256 and 2, 2, 3, 3
 *		 convolutions respectively.
 *		 Convolutions are made via Winograd. Float32 is used.  	
 *		 This implementation performs the batch normalization.  
 *
 * Parameter:   
 *		- const dim3 &dimsImg: Dimensions of the resized input image 
 *		- const dim3 &dimsFilter: Dimensions of the filters
 ...
 *
 * */
void SegnetImplementationOriginalWinograd2x2Arch3(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
  ...
  /* **************  Encoder  ************** */	
	...
	for (int cnt_layer = NB_LAYERS-1; cnt_layer >= 0; cnt_layer--, layer_num--){
		...
		if(cnt_layer > 2){
			...
			wino_conv2x2_norm_ReLu_CUDA<256, 256> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_wino_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_mean_decoder[conv_num], d_var_decoder[conv_num], d_weights_decoder[conv_num], d_bias_norm_decoder[conv_num], EPSILON);
		}else if(cnt_layer > 1){
			wino_conv2x2_norm_ReLu_CUDA<256, 256> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_wino_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_mean_decoder[conv_num], d_var_decoder[conv_num], d_weights_decoder[conv_num], d_bias_norm_decoder[conv_num], EPSILON);
...
/* SegnetImplementationIntegratedWinograd2x2Arch3
 *
 * Description: Executes a modified Segnet CNN considering 4 enconder-decoder layers
 *		 with output channels of 64, 128, 256, 256 and 2, 2, 3, 3
 ...
 *
 * */
void SegnetImplementationIntegratedWinograd2x2Arch3(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
  ...
  for (int j = 0; j < nIter; j++){
	...
	for (int cnt_layer = NB_LAYERS-1; cnt_layer >= 0; cnt_layer--, layer_num--){
		...
		if(cnt_layer > 2){
			...
			wino_conv2x2_ReLu_CUDA<256, 256> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_wino_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_bias_decoder[conv_num]);
		}else if(cnt_layer > 1){
			wino_conv2x2_ReLu_CUDA<256, 256> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_wino_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_bias_decoder[conv_num]);
