...
/*--------------------------- segnet_arch9_implementations.h ------------------------
|  File segnet_arch9_implementations.h
|
|  Description: Implementations of our 9th Segnet architecture 
|
|  Version: 1.0
*-----------------------------------------------------------------------------------*/

#ifndef SEGNET_ARCH9_IMPLEMENTATIONS_H_
#define SEGNET_ARCH9_IMPLEMENTATIONS_H_


//#define EVENT_RECORD_TIME 


/* SegnetImplementationOriginalTraditionalArch9
 *
 * Description: Executes a modified Segnet CNN considering 3 enconder-decoder layers 
 *		 and output channels of 64, 96 and 192 and 2, 3 and 3 convolutions respectively.
 ...
 *
 * */
void SegnetImplementationOriginalTraditionalArch9(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
  ...
  const unsigned NB_CONVs = 8; 
  // Normalization epsilon value
  const float EPSILON = 0.00001;
  
  // Convolutions per encoder/decoder layer
  const unsigned NB_CONV_LAYER1 = 2;
  const unsigned NB_CONV_LAYER2 = 3;
  const unsigned NB_CONV_LAYER3 = 3;

  const unsigned CONV_2_LAYER[NB_CONVs] = {0, 0, 1, 1, 1, 2, 2, 2};
  const unsigned CONV_PER_LAYER[NB_LAYERS] = {NB_CONV_LAYER1, NB_CONV_LAYER2, NB_CONV_LAYER3};
  ...
  strcat(bin_file_path_en_conv[4], "23_EncoderConv.bin");
  strcat(bin_file_path_en_conv[5], "31_EncoderConv.bin");
  strcat(bin_file_path_en_conv[6], "32_EncoderConv.bin");
  strcat(bin_file_path_en_conv[7], "33_EncoderConv.bin"); 
  ...
  strcat(bin_file_path_de_conv[4], "23_DecoderConv.bin");
  strcat(bin_file_path_de_conv[5], "31_DecoderConv.bin");
  strcat(bin_file_path_de_conv[6], "32_DecoderConv.bin");
  strcat(bin_file_path_de_conv[7], "33_DecoderConv.bin");
  ...
  strcat(bin_file_path_en_norm[4], "23_EncoderBatchNorm.bin");
  strcat(bin_file_path_en_norm[5], "31_EncoderBatchNorm.bin");
  strcat(bin_file_path_en_norm[6], "32_EncoderBatchNorm.bin");
  strcat(bin_file_path_en_norm[7], "33_EncoderBatchNorm.bin");
  ...
  strcat(bin_file_path_de_norm[4], "23_DecoderBatchNorm.bin");
  strcat(bin_file_path_de_norm[5], "31_DecoderBatchNorm.bin");
  strcat(bin_file_path_de_norm[6], "32_DecoderBatchNorm.bin");
  strcat(bin_file_path_de_norm[7], "33_DecoderBatchNorm.bin");
  ...
  unsigned nb_filters_encoder = 0;
  unsigned nb_norm_encoder = 0;
  ...
  unsigned nb_filters_decoder = 0;
  unsigned nb_norm_decoder = 0;
    ...
    for (int j = 0; j < NB_CONVs; j++){
    	...
    	nb_filters_encoder += fps_encoder[j].in_channels * fps_encoder[j].out_channels;  
    	nb_norm_encoder += size_norm_encoder[j];
   	...
   	size_filters_decoder[j] = fps_decoder[j].in_channels * fps_decoder[j].out_channels * fps_decoder[j].kernel_size_cols * fps_decoder[j].kernel_size_rows;
    	...
    	nb_filters_decoder += fps_decoder[j].in_channels * fps_decoder[j].out_channels;
    	nb_norm_decoder += size_norm_decoder[j];
    }
  ...
  unsigned pooled_index[NB_LAYERS] = {1, 4, 7};
  ...
  unsigned threads_per_block_conv[NB_LAYERS] = {64, 96, 192};
  ...
  for (int j = 0; j < nIter; j++){
	...
	convA_norm_ReLu_CUDA<64, 3, 64> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_features_encoder_l[conv_num-1], d_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num],  fps_encoder[conv_num].in_channels,  fps_encoder[conv_num].out_channels, d_mean_encoder[conv_num],  d_var_encoder[conv_num],  d_weights_encoder[conv_num], d_bias_norm_encoder[conv_num],  EPSILON);
	...
	for (int cnt_layer = 1; cnt_layer < NB_LAYERS; cnt_layer++){
		...
		if(layer_num < 2)
			convA_norm_ReLu_CUDA<96, 3, 96> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_max_pooled[layer_num-1], d_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels,  fps_encoder[conv_num].out_channels, d_mean_encoder[conv_num], d_var_encoder[conv_num], d_weights_encoder[conv_num], d_bias_norm_encoder[conv_num], EPSILON);
		else if(layer_num < 3)
			convA_norm_ReLu_CUDA<192, 3, 192> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_max_pooled[layer_num-1], d_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels,  fps_encoder[conv_num].out_channels, d_mean_encoder[conv_num], d_var_encoder[conv_num], d_weights_encoder[conv_num], d_bias_norm_encoder[conv_num], EPSILON);				
	...
	for (int cnt_layer = NB_LAYERS-1; cnt_layer >= 0; cnt_layer--, layer_num--){
		...
		if(cnt_layer > 1){
			convA_norm_ReLu_CUDA<192, 3, 192> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_max_unpooled[layer_num], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_mean_decoder[conv_num], d_var_decoder[conv_num], d_weights_decoder[conv_num], d_bias_norm_decoder[conv_num], EPSILON);
		}else if(cnt_layer > 0){
			convA_norm_ReLu_CUDA<96, 3, 96> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_max_unpooled[layer_num], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_mean_decoder[conv_num], d_var_decoder[conv_num], d_weights_decoder[conv_num], d_bias_norm_decoder[conv_num], EPSILON);
		}else{
			convA_norm_ReLu_CUDA<64, 3, 64> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_max_unpooled[layer_num], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_mean_decoder[conv_num], d_var_decoder[conv_num], d_weights_decoder[conv_num], d_bias_norm_decoder[conv_num], EPSILON);
		...
		if(cnt_layer > 1){
			...
			convA_norm_ReLu_CUDA<96, 3, 96> <<<blocks_per_grid_conv[conv_num-1], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_mean_decoder[conv_num], d_var_decoder[conv_num], d_weights_decoder[conv_num], d_bias_norm_decoder[conv_num], EPSILON);
			...
			convA_norm_ReLu_CUDA<64, 3, 64> <<<blocks_per_grid_conv[conv_num-1], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_mean_decoder[conv_num], d_var_decoder[conv_num], d_weights_decoder[conv_num], d_bias_norm_decoder[conv_num], EPSILON);
		}else{
...
/* SegnetImplementationIntegratedTraditionalArch9
 *
 * Description: Executes the Segnet CNN  enconder-decoder layers and output 
 *		 channels of 64, 96 and 192 and 2, 3 and 3 convolutions respectively.
 *		 Convolutions are performed using the traditional convolution operation. 	
 ...
 *
 * */
void SegnetImplementationIntegratedTraditionalArch9(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
  ...
  strcat(bin_file_path_en_conv[4], "23_EncoderIntegratedConvAndBatchNorm.bin");
  strcat(bin_file_path_en_conv[5], "31_EncoderIntegratedConvAndBatchNorm.bin");
  strcat(bin_file_path_en_conv[6], "32_EncoderIntegratedConvAndBatchNorm.bin");
  strcat(bin_file_path_en_conv[7], "33_EncoderIntegratedConvAndBatchNorm.bin");
  ...
  strcat(bin_file_path_de_conv[4], "23_DecoderIntegratedConvAndBatchNorm.bin");
  strcat(bin_file_path_de_conv[5], "31_DecoderIntegratedConvAndBatchNorm.bin");
  strcat(bin_file_path_de_conv[6], "32_DecoderIntegratedConvAndBatchNorm.bin");
  strcat(bin_file_path_de_conv[7], "33_DecoderIntegratedConvAndBatchNorm.bin");
  ...
  for (int j = 0; j < nIter; j++){
	...
	// Perform image*filter convolution 
	convA_ReLu_CUDA<64, 3, 64> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_features_encoder_l[conv_num-1], d_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num],  fps_encoder[conv_num].in_channels,  fps_encoder[conv_num].out_channels, d_bias_encoder[conv_num]);
	...
	for (int cnt_layer = 1; cnt_layer < NB_LAYERS; cnt_layer++){
		...
		// Perform convolution after downsampling
		if(layer_num < 2)
			convA_ReLu_CUDA<96, 3, 96> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_max_pooled[layer_num-1], d_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels,  fps_encoder[conv_num].out_channels, d_bias_encoder[conv_num]);
		else if(layer_num < 3)
			convA_ReLu_CUDA<192, 3, 192> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_max_pooled[layer_num-1], d_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels,  fps_encoder[conv_num].out_channels, d_bias_encoder[conv_num]);				
	...
	for (int cnt_layer = NB_LAYERS-1; cnt_layer >= 0; cnt_layer--, layer_num--){
		...
		if(cnt_layer > 1){
			...
			convA_ReLu_CUDA<96, 3, 96> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_max_unpooled[layer_num], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_bias_decoder[conv_num]);
		}else{
			convA_ReLu_CUDA<64, 3, 64> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_max_unpooled[layer_num], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_bias_decoder[conv_num]);
		...
		if(cnt_layer > 1){
			...
			convA_ReLu_CUDA<96, 3, 96> <<<blocks_per_grid_conv[conv_num-1], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_bias_decoder[conv_num]);
			...
			convA_ReLu_CUDA<64, 3, 64> <<<blocks_per_grid_conv[conv_num-1], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_bias_decoder[conv_num]);
		}else{
...
/* SegnetImplementationOriginalWinograd2x2Arch9
 *
 * Description: Executes the Segnet CNN  enconder-decoder layers and output 
 *		 channels of 64, 96 and 192 and 2, 3 and 3 convolutions respectively.
 ...
 *
 * */
void SegnetImplementationOriginalWinograd2x2Arch9(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
  ...
  for (int i = 0; i < NB_CONVs; i++){
  	if(size_Img_Y[CONV_2_LAYER[i]] % 2 == 0)
  		blocks_per_grid_conv[i] = ceil((float)(fps_encoder[i].out_channels*size_Img_X[CONV_2_LAYER[i]]*size_Img_Y[CONV_2_LAYER[i]])/(float)(WINO_TILE_SIZE*threads_per_block_conv[CONV_2_LAYER[i]])); 
	else
		blocks_per_grid_conv[i] = ceil((float)(fps_encoder[i].out_channels*size_Img_X[CONV_2_LAYER[i]]*(size_Img_Y[CONV_2_LAYER[i]]+1))/(float)(WINO_TILE_SIZE*threads_per_block_conv[CONV_2_LAYER[i]]));
  ...
  for (int j = 0; j < nIter; j++){
	...
	for (int cnt_layer = 1; cnt_layer < NB_LAYERS; cnt_layer++){
		...
		if(layer_num < 2)
			wino2b_conv2x2_norm_ReLu_CUDA<96, 64> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_max_pooled[layer_num-1], d_wino_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels, fps_encoder[conv_num].out_channels, d_mean_encoder[conv_num], d_var_encoder[conv_num], d_weights_encoder[conv_num], d_bias_norm_encoder[conv_num], EPSILON);
		else if(layer_num < 3)
			wino2_conv2x2_norm_ReLu_CUDA<96, 96> <<<2*blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_max_pooled[layer_num-1], d_wino_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels, fps_encoder[conv_num].out_channels, d_mean_encoder[conv_num], d_var_encoder[conv_num], d_weights_encoder[conv_num], d_bias_norm_encoder[conv_num], EPSILON);
		...
		for (int cnt_conv_layer = 0; cnt_conv_layer < CONV_PER_LAYER[cnt_layer]-1; cnt_conv_layer++){
			...
			if(layer_num < 2)
				wino_conv2x2_norm_ReLu_CUDA<96, 96> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_features_encoder_l[conv_num-1], d_wino_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels, fps_encoder[conv_num].out_channels, d_mean_encoder[conv_num], d_var_encoder[conv_num], d_weights_encoder[conv_num], d_bias_norm_encoder[conv_num], EPSILON);
			else if(layer_num < 3)
				wino_conv2x2_norm_ReLu_CUDA<192, 192> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_features_encoder_l[conv_num-1], d_wino_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels, fps_encoder[conv_num].out_channels, d_mean_encoder[conv_num], d_var_encoder[conv_num], d_weights_encoder[conv_num], d_bias_norm_encoder[conv_num], EPSILON);
	...
	for (int cnt_layer = NB_LAYERS-1; cnt_layer >= 0; cnt_layer--, layer_num--){
		...
		if(cnt_layer > 1){
			...
			wino_conv2x2_norm_ReLu_CUDA<96, 96> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_max_unpooled[layer_num], d_wino_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_mean_decoder[conv_num], d_var_decoder[conv_num], d_weights_decoder[conv_num], d_bias_norm_decoder[conv_num], EPSILON);
		}else{
		...
		/* **************  Deconvolution Loop ************** */
		 if(cnt_layer > 1){
			...
			wino3_conv2x2_norm_ReLu_CUDA<96, 192> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_wino_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_mean_decoder[conv_num], d_var_decoder[conv_num], d_weights_decoder[conv_num], d_bias_norm_decoder[conv_num], EPSILON);
		}else if(cnt_layer > 0){
			wino_conv2x2_norm_ReLu_CUDA<96, 96> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_wino_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_mean_decoder[conv_num], d_var_decoder[conv_num], d_weights_decoder[conv_num], d_bias_norm_decoder[conv_num], EPSILON);
		
			conv_num--;
		
			wino3b_conv2x2_norm_ReLu_CUDA<64, 96> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_wino_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_mean_decoder[conv_num], d_var_decoder[conv_num], d_weights_decoder[conv_num], d_bias_norm_decoder[conv_num], EPSILON);
		}else{
			wino4_conv2x2_norm_ReLu_CUDA <<<ceil((float)(NB_CLASSES*size_Img_X[layer_num]*size_Img_Y[layer_num]/(float)(WINO_TILE_SIZE*256))), 256, 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_wino_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_mean_decoder[conv_num], d_var_decoder[conv_num], d_weights_decoder[conv_num], d_bias_norm_decoder[conv_num], EPSILON);
		}
 ...
 /* SegnetImplementationIntegratedWinograd2x2Arch9
 *
 * Description: Executes the Segnet CNN  enconder-decoder layers and output 
 *		 channels of 64, 96 and 192 and 2, 3 and 3 convolutions respectively.
 *		 Convolutions are made via Winograd. The filter weights integrate 
 *		 the batch normalization. Float32 is used.	
 *
 * Parameter:   
 ...
 *
 * */
void SegnetImplementationIntegratedWinograd2x2Arch9(const dim3 &dimsImg, const dim3 &dimsFilter, const char INPUT_IMAGE_ADDR[], const char BIN_BASE_ADDR[], const char OUTPUT_IMAGE_ADDR[]){
  ...
  for (int j = 0; j < nIter; j++){
	...
	for (int cnt_layer = 1; cnt_layer < NB_LAYERS; cnt_layer++){
		...
		// Perform image*filter convolution for the current feature
		if(layer_num < 2)
			wino2b_conv2x2_ReLu_CUDA<96, 64> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_max_pooled[layer_num-1], d_wino_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels, fps_encoder[conv_num].out_channels, d_bias_encoder[conv_num]);
		else if(layer_num < 3)
			wino2_conv2x2_ReLu_CUDA<96, 96> <<<2*blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_max_pooled[layer_num-1], d_wino_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels, fps_encoder[conv_num].out_channels, d_bias_encoder[conv_num]);
		...
		for (int cnt_conv_layer = 0; cnt_conv_layer < CONV_PER_LAYER[cnt_layer]-1; cnt_conv_layer++){
			...
			if(layer_num < 2)
				wino_conv2x2_ReLu_CUDA<96, 96> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_features_encoder_l[conv_num-1], d_wino_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels, fps_encoder[conv_num].out_channels, d_bias_encoder[conv_num]);
			else if(layer_num < 3)
				wino_conv2x2_ReLu_CUDA<192, 192> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_encoder_l[conv_num], d_features_encoder_l[conv_num-1], d_wino_filters_encoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_encoder[conv_num].in_channels, fps_encoder[conv_num].out_channels, d_bias_encoder[conv_num]);
	...
	for (int cnt_layer = NB_LAYERS-1; cnt_layer >= 0; cnt_layer--, layer_num--){
		...
		if(cnt_layer > 1){
			...
			wino_conv2x2_ReLu_CUDA<96, 96> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_max_unpooled[layer_num], d_wino_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_bias_decoder[conv_num]);
		...
		if(cnt_layer > 1){
			...
			wino3_conv2x2_ReLu_CUDA<96, 192> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_wino_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_bias_decoder[conv_num]);
		}else if(cnt_layer > 0){
			wino_conv2x2_ReLu_CUDA<96, 96> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_wino_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_bias_decoder[conv_num]);
			
			conv_num--;
			
			wino3b_conv2x2_ReLu_CUDA<64, 96> <<<blocks_per_grid_conv[conv_num], threads_per_block_conv[layer_num-1], 0, m0_stream>>>(d_features_decoder_l[conv_num], d_features_decoder_l[conv_num+1], d_wino_filters_decoder[conv_num], size_Img_X[layer_num], size_Img_Y[layer_num], fps_decoder[conv_num].in_channels, fps_decoder[conv_num].out_channels, d_bias_decoder[conv_num]);
		}else{
