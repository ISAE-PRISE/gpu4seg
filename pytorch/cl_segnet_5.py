...
class cl_segnet_5(torch_nn.Module):

	def __init__(self, in_chanel_size, out_chanel_size, bn_momentum):
		super(cl_segnet_5, self).__init__()
		...
		self.ConvEn23 = torch_nn.Conv2d(self.chn_2, self.chn_2, kernel_size=3, padding=1, bias=False)
		self.BNEn23 = torch_nn.BatchNorm2d(self.chn_2, momentum=bn_momentum)
		...
		self.ConvDe23 = torch_nn.Conv2d(self.chn_2, self.chn_2, kernel_size=3, padding=1, bias=False)
		self.BNDe23 = torch_nn.BatchNorm2d(self.chn_2, momentum=bn_momentum)
		...
		torch.nn.init.kaiming_uniform_(self.ConvEn23.weight)
		...
		torch.nn.init.kaiming_uniform_(self.ConvDe23.weight)
	...
	def forward(self, x):
		...
		x_en2_res = x # residual
		x = torch_nn_func.relu(self.BNEn22(self.ConvEn22(x))) 
		x = torch_nn_func.relu(self.BNEn23(self.ConvEn23(x))+x_en2_res) 
		...
		x_en3_res = x # residual
		x = torch_nn_func.relu(self.BNEn32(self.ConvEn32(x))) 
		x = torch_nn_func.relu(self.BNEn33(self.ConvEn33(x))+x_en3_res) 
		...
		x_de3_res = x # residual
		x = torch_nn_func.relu(self.BNDe33(self.ConvDe33(x)))
		x = torch_nn_func.relu(self.BNDe32(self.ConvDe32(x))+x_de3_res)
		...
		x_de2_res = x # residual
		x = torch_nn_func.relu(self.BNDe23(self.ConvDe23(x)))
		x = torch_nn_func.relu(self.BNDe22(self.ConvDe22(x))+x_de2_res)
